...
static int run(const char *name, struct test *tests, size_t size, cli_val v)
{
	for (size_t i = 0; i < size; ++i) {
		...
		cli_val_print(v1, otrap_file(&otrap));
		...
		if (output_size != expect_size
			...
			fprintf(stderr, "%s: Expected %s + %s to equal %s but got ", name, test->value1, test->value2, test->result);
			cli_val_print(v1, stderr);
...
int main(void)
{
	...
	test_integer_8_2scmpl();
	test_integer_8_unsigned();
	test_integer_16_2scmpl();
	test_integer_16_unsigned();
	test_integer_32_2scmpl();
	test_integer_32_unsigned();
	test_integer_64_2scmpl();
